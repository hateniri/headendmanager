...
import { X, Upload, Trash2, AlertCircle, Download, FileSpreadsheet, FileText } from 'lucide-react'
...
export default function FireDetectionInspectionForm({ isOpen, onClose, facilityId, facilityName }: FireDetectionInspectionFormProps) {
  ...
  const calculateOverallJudgment = (): '良好' | '要注意' | '要修理' | '危険' => {
    ...
    if (formData.abnormalities && (
      ...
      formData.abnormalities.includes('警報') ||
    ...
    const hasCriticalFailure = 
      formData.alarmPanel.fireAlarmTest === '×' ||
      formData.alarmPanel.abnormalAlarmTest === '×' ||
      formData.alarmPanel.backupPowerTest === '×' ||
      formData.samplingSensor.fireTest === '×' ||
      formData.portableSensor.testGasResponse === '×'
    ...
    // 注意が必要な項目
    const hasWarning = 
      Object.values(formData.alarmPanel).includes('×') ||
      Object.values(formData.samplingSensor).includes('×') ||
      Object.values(formData.portableSensor).includes('×')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addHistory({
      facilityId,
      facilityName,
      inspectionType: 'fire-prediction',
  ...
  const handleExportExcel = () => {
    const excelData = [
      formatInspectionDataForExcel({ ...commonData, ...formData }, '火災予兆検知'),
      { 点検種別: formData.inspectionType },
      { 警報盤火災警報試験: formData.alarmPanel.fireAlarmTest },
      { サンプリングセンサ火災試験: formData.samplingSensor.fireTest },
      { ポータブルセンサガス応答: formData.portableSensor.testGasResponse },
    ]
    exportToExcel(excelData, `火災予兆検知システム点検報告書_${facilityName}_${commonData.inspectionStartDate}.xlsx`, '火災予兆検知')
  }
  ...
  const handleExportPDF = () => {
    ...
    const data = [
      ...
      { item: '点検種別', value: formData.inspectionType },
    ...
    exportToPDF(
      ...
      data,
      columns,
      `火災予兆検知システム点検報告書_${facilityName}_${commonData.inspectionStartDate}.pdf`,
      [
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b z-10">
          <div className="px-6 py-4">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">火災予兆検知システム点検報告書</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          {/* 1. 基本情報 */}
          <div className="mb-8">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">1. 火災予兆検知システム基本情報</h3>
